...
const EditClientModal = ({ visible, onClose, onSave, name, phoneNumber,email,password,id}) => {
    console.log(email)
    console.log(id)
...
console.log(id)
  ...
  if (checkValid) {
    try {
      const res = await actions.EditClient({
        id: id,
        email: editedEmail,
        name: editedName,
        password: editedPassword,
      ...
      onClose(); // Close the modal by calling the onClose prop
    } catch (error) {
